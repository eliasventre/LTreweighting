...
def plot_metrics(couplings, cost_func, cost_func_name, epsilons, log = False, points=False, scale=1.0, label_font_size=18, tick_font_size=12):
    ...
    if "OT" in couplings.keys():
        ...
        all_ys.append(ys)
    if "FGW" in couplings.keys():
        ys = np.array([cost_func(c) for c in [couplings['FGW entropic rna ' + str(e)] for e in epsilons]])
        plt.plot(epsilons, ys/scale, label = "DW, true tree")
        if points:
            plt.scatter([zero_offset], [cost_func(couplings["FGW"])/scale])
        all_ys.append(ys)
    ...
    # upper limit should be at least 1.1
    plt.ylim([-.1, max(ylims[1], 1.1)])
