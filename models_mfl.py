# This file is taken from https://github.com/zsteve/mfl, and contains the code for the denoted MFL algorithm
...
class PathsLoss(torch.nn.Module):
    def __init__(self, x, M, tau, dt, sinkhorn_iters, device, scale_factors, lamda_unbal = None, warm_start = False, branching_rate_fn = None):
        super(PathsLoss, self).__init__()
        self.device = device
        self.x = x # starting empirical marginals, dimensions (T, N, d)
        self.T = len(x) # number of timepoints
        ...
        self.ot_losses = torch.nn.ModuleList([SinkhornLoss(torch.full((self.N[i], ), 1/self.N[i], device = self.device),
                                                           self.x[i][:, :],
                                                           torch.full((self.N[i+1], ), 1/self.N[i+1], device = self.device),
                                                           self.x[i+1][:, :],
                                                           self.tau * dt,
                                                           self.sinkhorn_iters,
...
class TrajLoss(torch.nn.Module):
    def __init__(self, x0, weights, x_obs, t_idx_obs, dt, tau, sigma, M, lamda_reg, lamda_cst, sigma_cst, branching_rate_fn, sinkhorn_iters, device, lamda_unbal = None, warm_start = False, scale_factors = None, scale_factors_fit = None, scale_factor_global = None):
        ...
        self.w = (1 / len(self.w)) * self.w/self.w # .sum() # compute time-point weights
        self.warm_start = warm_start
        ...
        self.loss_fit = torch.nn.ModuleList([FitLoss(self.x[i][:, :], torch.full((self.M[i], ), 1/self.M[i], device = self.device),
...
def optimize(model, n_iter, tau_final, eta_final, sigma_final, temp_init, temp_ratio, dim, print_interval = 50, **kwargs):
    ...
    x_save = torch.zeros((n_iter, ) + (np.sum([model.x[j].shape[0] for j in range(0, len(model.x))]), dim))
    for i in range(n_iter):
        with torch.no_grad():
            x_save[i, :, :] = torch.cat([model.x[j].data.clone() for j in range(0, len(model.x))], 0)
        ## set noise level
